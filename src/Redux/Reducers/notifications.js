...
export default function (state, action) {
  const prependAdminMessage = (message) => {
    ...
    ].concat (state.news.adminMessages.slice(0,100));
  }
  ...
  case actionTypes.SEND_NOTIFICATIONS_RESULT:
    return {
      ...
      news: {
        ...state.news,
        adminMessages: prependAdminMessage (`Sent Email Notification for News Item ID ${action.payload.newsId} ${action.payload.responseText || ''}`)
      }
    };
  ...
  case actionTypes.SET_SEND_NOTIFICATIONS_STATUS:
    return {
      ...
      news: {
        ...state.news,
        adminMessages: prependAdminMessage (`Notification Status: ${action.payload.status} for News Item Id ${action.payload.newsId}`)
      }
    }
  ...
  case actionTypes.RESEND_NOTIFICATIONS:
    return {
      ...
      news: {
        ...state.news,
        adminMessages: prependAdminMessage (`Resent Notification for Email ID ${action.payload.newsId} and News ID ${action.payload.newsId}`),
      }
    }
  ...
  case actionTypes.SET_RESEND_NOTIFICATIONS_STATUS:
    return {
      ...
      news: {
        ...state.news,
        adminMessages: prependAdminMessage (`Resend Status: ${action.payload.status} for News ID ${action.payload.newsId} and Email ID ${action.payload.emailId}`),
      }
    }

...
class AdminDashboard extends Component {
  constructor(props) {
    ...
    let paramsIncludeFilterStates = Object.keys(params).some(
      (e) => initialFilterState[e] !== undefined,
    );
    ...
    let filters = Object.fromEntries(
      Object.keys(initialFilterState).map((e) => [
        e,
        paramsIncludeFilterStates
          ? params[e] === 'true'
          : initialFilterState[e],
      ]),
    );
  ...
  componentDidUpdate = (prevProps) => {
    if (
      !(prevProps.dataSubmissions && prevProps.dataSubmissions.length) &&
      this.props.dataSubmissions &&
      this.props.dataSubmissions.length
    ) {
  ...
  handleChangeFilter = (e) => {
    ...
    this.props.history.push(
      `/datasubmission/admindashboard?${queryString.stringify(newFilterState)}`,
    );
  };
  ...
  render = () => {
    ...
    let submissions = searchString
      ? search
          .search(searchString)
          .filter((item) => this.state.filters[item.Phase])
    ...
    return (
      <div className={classes.wrapperDiv}>
        ...
        >
          <DialogTitle>
            Deleting {deleteTarget ? deleteTarget.Dataset : ''}
          </DialogTitle>

          <DialogContent>
            This action will permanently delete all database records of this
            submission including references to all uploaded versions, and
            comments. Workbooks in dropbox will not be affected.
          </DialogContent>
          ...
          <React.Fragment>
            ...
            >
              <AccordionSummary expandIcon={<ExpandMore />}>
                <Typography
                  noWrap
                  className={classes.panelSummaryText}
                  style={{ flexBasis: '23%' }}
                >
                  Dataset Long Name
                </Typography>
              ...
              >
                <AccordionSummary expandIcon={<ExpandMore />}>
                  ...
                  <Typography
                    ...
                    style={{ flexBasis: '10%' }}
                  >
                    {e.Ingestion_Date_Time
                      ? e.Ingestion_Date_Time.slice(0, 10)
                      : 'NA'}
                  </Typography>
                </AccordionSummary>

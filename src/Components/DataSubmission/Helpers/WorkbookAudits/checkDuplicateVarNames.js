import auditFactory, { requireVars, makeIssueList } from './auditFactory';
...
const check = (standardAuditArgs) => {
  ...
  const duplicateShortNames = checkValuesAreUnique(
    vars_meta_data,
    'var_short_name',
  );
  if (duplicateShortNames && duplicateShortNames.length) {
    results.push(
      makeIssueList(severity.error, 'Duplicate Variable Short Names', {
  ...
  const duplicateLongNames = checkValuesAreUnique(
    vars_meta_data,
    'var_long_name',
  );
  if (duplicateLongNames && duplicateLongNames.length) {
    results.push(
      makeIssueList(severity.error, 'Duplicate Variable Long Names', {

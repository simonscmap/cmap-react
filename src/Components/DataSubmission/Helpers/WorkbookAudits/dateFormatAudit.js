...
const checkDateFormat = (standardArgs) => {
  const { data, is1904, numericDateFormatConverted } = standardArgs;
  ...
  const { negativeNumberDate, integerDate, missingDate } =
  ...
  if (!timeValuesAreConsistent) {
    return {
      error: {
        ...
        detail:
          'Time values in the data sheet are not all the same type. Please check your data sheet to ensure all dates are of the same type and format.',
      },
    };
  }

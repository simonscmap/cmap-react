...
const DESCRIPTION =
  'Check user defined variables have matching column in data sheet';
...
const check = (standardAuditArgs) => {
  ...
  const unmatchedShortNames = varsShortNames.filter(
    (e) => !userVariables.has(e),
  );

  const colsWithoutVarDefs = Array.from(userVariables).filter(
    (c) => !varsShortNames.includes(c),
  );
  ...
  if (colsWithoutVarDefs.length) {
    results.push(
      makeIssueList(severity.error, 'Unidentified Columns in the Data Sheet', {

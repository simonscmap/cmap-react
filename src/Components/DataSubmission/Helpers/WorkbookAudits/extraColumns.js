...
const DESCRIPTION = 'Check metadata and variable sheets for non-prescribed column names';
...
const check = (standardAuditArgs) => {
  ...
  // check
  const extraMetaColumns = checkExtraColumns (dataset_meta_data, 'dataset_meta_data');
  if (extraMetaColumns) {
    results.push (makeIssueList (
      severity.error,
      'Extra Columns in Dataset Metadata Sheet',
      {

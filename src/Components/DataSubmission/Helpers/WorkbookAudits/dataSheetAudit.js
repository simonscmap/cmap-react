...
export const reportDuplicateDataColumnHeaders = (workbook, dataSheet) => {
  ...
  const findOriginalColumnLetter = (header) => {
    const result = Object.keys(workbook.Sheets.data).filter(
      (k) => header === safePath([k, 'h'])(workbook.Sheets.data),
    );
    return result;
  };
  ...
  const matchedHeaders = parsedHeaders.reduce((acc, parsedValue) => {
    ...
    if (
      maybeDuplicate &&
      duplicateHeadersFromWorkbook.includes(maybeDuplicate)
    ) {
        ...
        IssueWithList,
        {
          ...
          list: Object.entries(duplicateHeaders).map(([header, headerData]) => {
            const originalCols = headerData.maybeOriginalColumns.length
              ? headerData.maybeOriginalColumns
                  .map((v) => `*\`${v}\`*`)
                  .join(', ')
              : '';
            const str = originalCols.length
              ? ` (originally in columns ${originalCols})`
              : '';

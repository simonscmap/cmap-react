...
const handleSectionMap = (
  ...
  const distinctMeridiansOrParallelsForSplitting =
    orientation === 'zonal' ? lats : lons;

  const xAxis = orientation === 'zonal' ? 'Longitude' : 'Latitude';

  let xTicks =
    parameters.lon1 > parameters.lon2 && orientation === 'zonal'
  ...
  return subsets.map((subset, index) => {
    ...
    const latTitle =
      orientation === 'meridional'
        ...
        : splitBySpace
        ? distinctMeridiansOrParallelsForSplitting[
            index % distinctMeridiansOrParallelsForSplitting.length
          ] + '\xb0'
        ...
        data={[
          {
            ...
            type:
              infoObject.subType === visualizationSubTypes.contourSectionMap
                ? 'contour'
                : 'heatmap',
            colorbar: {
              title: {
...
const SectionMapChart = (props) => {
  ...
  // switchOrientation does not appear to be used by ChartControlPanel at all

...
const handleContourMap = (
  ...
  const depths = Array.from(infoObject.depths).map((depth) =>
    parseFloat(depth),
  );
  ...
  let xTicks =
    infoObject.parameters.lon1 > infoObject.parameters.lon2
      ? handleXTicks(infoObject)
      : {};
  ...
  return subsets.map((subset, index) => {
    const date =
      dates.length <= 1
        ...
        : splitByDepth
        ? handleChartDateString(
            dates[Math.floor(index / depths.length)],
            infoObject.hasHour,
            infoObject.isMonthly,
          )
        : handleChartDateString(
    ...
    const depth = !infoObject.hasDepth
      ...
      : splitByDate
      ? depths[index % depths.length].toFixed(2) + '[m]'
      : depths[index].toFixed(2) + '[m]';
    ...
    var hovertext = subset.map((value, i) => {
      ...
      if (value === null)
        return (
          `Lat: ${format('.2f')(infoObject.lats[i])}\xb0` +
          `<br>` +
          `Lon: ${
            infoObject.lons[i] > 180
              ? format('.2f')(infoObject.lons[i] - 360)
              : format('.2f')(infoObject.lons[i])
          }\xb0`
        );
      ...
      return (
        ...
        '<br>' +
        `${infoObject.parameters.fields}: ${format(formatter)(value)} [${
          infoObject.metadata.Unit
        }]`
      );
    });
        ...
        data={[
          {
            ...
            contours: {
              ...
              labelformat:
                infoObject.zMin > 1 && infoObject.zMin < 1000 ? '.2f' : '.2e',
            },
            colorbar: {
              title: {
...
const SpaceTimeChart = (props) => {
  ...
  return (
    <React.Fragment>
      <ChartControlPanel
        handlePaletteChoice={
          subType !== subTypes.histogram && handlePaletteChoice
        }
        ...
        handleZValueConfirm={
          subType !== subTypes.histogram && handleZValueConfirm
        }

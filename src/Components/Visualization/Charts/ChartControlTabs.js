        ...
        {charts.map((e, i) => (
          <Tooltip
            ...
            title={
              ...
              ' from ' +
              e.data.metadata.Dataset_Name
            }
          >
            <Tab
              label={
                <span style={{ maxWidth: '140px' }}>
                  {e.data.metadata.Long_Name.length > 32
                    ? e.data.metadata.Long_Name.slice(0, 30) + '...'
                    : e.data.metadata.Long_Name}

                  <Tooltip title="Delete plot" placement="top">
                    <Close
                      className={classes.closeIcon}
                      onClick={(e) => handleDeleteChart(e, i)}
                    />
                  </Tooltip>
                </span>
              }
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withStyles(styles)(ChartControlTabs));

  ...
  return (
    <div className={classes.vumContainer}>
      {data.map((vumBlob, blobIdx) => {
        ...
        return (
          <div
            className={classes.vumBlob}
            onClick={() => handleDispatch(vumBlob)}
            key={`vumBlob-${blobIdx}`}
          >
            {keys.map((key, keyIdx) => {
              ...
              return (
                <div className={classes.blob} key={`blob-${key}`}>
                  <div className={classes.blobKeyHeading}>
                  ...
                  {zipped.map(([value, description], idx) => {
                    return (
                      <div
                        className={classes.blobValuesContainer}
                        key={`blob-${key}(${keyIdx})-blob${idx}`}
                      >
                        <div className={classes.blobKeyV}>

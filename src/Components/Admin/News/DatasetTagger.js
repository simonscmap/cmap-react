...
const DatasetListItem = withStyles({
  ...
  return <ListItemText primary={name} className={classes.listItem} />;
});

const tail = (arg) =>
  Array.isArray(arg) && arg.length > 0 && arg[arg.length - 1];
...
const findDiscrepancies = (story, tags, datasetNames) => {
  ...
  const add = (item) =>
    typeof item === 'string' && words.add(item.toLowerCase());
  ...
  const missingTags = links
    ...
    .map(
      (lastSegment) =>
        lastSegment && lastSegment.toLowerCase && lastSegment.toLowerCase(),
    )
    .concat([linkShortName])
    .filter((ls) => !tags.some((t) => t.toLowerCase() === ls)) // narrow to links that are not tagged
    .filter((ls) =>
      datasetNames.find(({ shortName }) => shortName.toLowerCase() === ls),
    ) // narrow to dataset names
...
const Tags = (props) => {
  const { handleAdd, handleDelete, tags } = props;
          ...
          if (tag.unmentioned) {
            return (
              <React.Fragment key={`tag${ix}`}>
                <Tooltip
                  title={`This short name is tagged, but not mentioned in the news item.`}
            ...
            return (
              <React.Fragment key={`missing${ix}`}>
                <Tooltip
                  title={
                    'This short name is mentioned in the news item, but is not yet tagged. Click to add.'
                  }
                  placement="bottom-end"
                >
                  <MissingChip
                    label={`(+) ${tag.t}`}
                    onClick={handleAdd(tag.t)}
                    className={cl.chip}
                  />
                </Tooltip>
              </React.Fragment>
            ...
            return (
              <React.Fragment key={`tag${ix}`}>
                <Tooltip
                  title={`This short name is "tagged": it is associated with this news item.`}
                  placement="bottom-end"
                >
                  <Chip
                    className={cl.chip}
...
const Tagger = (props) => {
  ...
  useEffect(() => {
    if (searchTerm === '') {
      ...
      const matches = datasetNames.filter(({ shortName }) =>
    ...
    <div className={cl.container}>
      ...
      <div className={cl.manager}>
        ...
        <div className={cl.listContainer}>
          <List>
            {filteredDatasets &&
              filteredDatasets.map(({ shortName }, ix) => {
                ...
                return (
                  ...
                  >
                    <DatasetListItem
                      name={shortName}
                      isSelected={thisDatasetIsTagged}
                    />
                  </ListItem>
                );
              })}

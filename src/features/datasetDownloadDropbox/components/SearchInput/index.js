...
const SearchInput = ({ selectedFiles = [], onToggleFile }) => {
  ...
  const searchResults = useSelector((state) =>
    selectSearchResults(state, currentTab),
  );
  const isSearchActive = useSelector((state) =>
    selectIsSearchActive(state, currentTab),
  );
          ...
          searchResults.length > 0 && (
            ...
            >
              ...
              >
                <Typography
                  variant="body2"
                  style={{ color: '#9dd162', fontWeight: 500 }}
                >
              ...
              {/* File list */}
              {searchResults
                .slice(0, SEARCH_RESULTS_DISPLAY_LIMIT)
                .map((file, index) => {
              ...
              {searchResults.length > SEARCH_RESULTS_DISPLAY_LIMIT && (
                <Box
                  ...
                  py={1}
                  textAlign="center"
                  style={{ borderTop: '1px solid #2c6b8f' }}
                >
                  <Typography
                    variant="caption"
                    style={{ color: '#9dd162', fontStyle: 'italic' }}
                  >
                    and {searchResults.length - SEARCH_RESULTS_DISPLAY_LIMIT}{' '}
                    more files...
                  </Typography>
                </Box>
              )}

/**
 * Row Count Estimation - Pipeline with Phases Pattern
 *
...
function clampSpatialBounds(queryBounds, datasetBounds) {
  ...
  if (queryCrossesDateline) {
    // For dateline-crossing query, check which segment(s) the dataset overlaps
    // East segment: [queryLonMin, 180], West segment: [-180, queryLonMax]
    ...
    if (overlapsEast && overlapsWest) {
      // Dataset overlaps both segments - return crossing bounds
      ...
      // Only east segment overlaps - return normal (non-crossing) bounds
      clampedLonMin = Math.max(queryBounds.lonMin, datasetBounds.lonMin);
      clampedLonMax = datasetBounds.lonMax;
    } else {
      // Only west segment overlaps - return normal (non-crossing) bounds
      clampedLonMin = datasetBounds.lonMin;
      clampedLonMax = Math.min(queryBounds.lonMax, datasetBounds.lonMax);
    }
  } else {
    // Normal case: intersection is max of mins, min of maxes
    clampedLonMin = Math.max(queryBounds.lonMin, datasetBounds.lonMin);
    clampedLonMax = Math.min(queryBounds.lonMax, datasetBounds.lonMax);
  }
...
const longitudeRules = [
  // CALCULATE: Date line crossing
  {
    ...
    apply: (value, inputs, effective) => {
      ...
      const countToDateLine = countSpatialDataPoints(lonMin, eastEnd, effectiveLonMin, resolutions.spatialDegrees);
      const countFromDateLine = countSpatialDataPoints(westStart, lonMax, effectiveLonMin, resolutions.spatialDegrees);
 ...
 */
function performRowCountMathPhased(resolvedInputs, constraints) {
...
export default performRowCountMathPhased;

...
import { calculateRowCounts as calculateRowCountsApi } from '../api/rowCountApi';
...
export async function calculateRowCountsForDatasets(
  datasets,
  constraints,
  skippedDatasets = [],
) {
  ...
  try {
    ...
    if (shortNames.length > 0) {
      ...
      const response = await calculateRowCountsApi(
        shortNames,
        backendConstraints,
        controller.signal,
      );
 ...
 */
export async function recalculateStaleDatasets(results, constraints) {
  ...
  await calculateRowCountsForDatasets(datasetsToRecalculate, constraints);
}
 ...
 */
export async function calculateRowCounts(datasets, constraints) {
  if (!datasets || datasets.length === 0) {
    useRowCountCalculationStore.setState({ staleDatasets: [] });
    return;
  }

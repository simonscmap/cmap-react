import DataExportService from '../../src/shared/services/dataDownload/dataExportService';
import DownloadService from '../../src/shared/services/dataDownload/downloadService';
import apiService from '../../api/api';

// Mock dependencies
jest.mock('../../src/shared/services/dataDownload/downloadService');
jest.mock('../../api/api');
...
describe('DataExportService', () => {
  ...
  describe('exportDataset', () => {
    ...
    beforeEach(() => {
      ...
      jest.mock(
        '../../src/shared/services/dataDownload/datasetMetadataToDownloadFormat',
        () => ({
    ...
    it('should fetch data and metadata then create ZIP', async () => {
      ...
      expect(apiService.catalog.datasetMetadata).toHaveBeenCalledWith(
        'test_table',
      );
    ...
    it('should handle errors gracefully', async () => {
      ...
      await expect(
        DataExportService.exportDataset({ query: mockQuery }),
  ...
  describe('fetchDatasetMetadata', () => {
    ...
    it('should handle API errors', async () => {
      ...
      await expect(
        DataExportService.fetchDatasetMetadata({ tableName: 'test' }),
  ...
  describe('convertVisualizationDataToCSV', () => {
    it('should convert visualization data to CSV', () => {
      const data = [{ time: '2023-01-01', value: 100 }];

      jest
        .spyOn(DataExportService, 'normalizeVisualizationData')
        .mockReturnValue(data);
      jest
        .spyOn(DataExportService, 'determineVisualizationColumns')
        .mockReturnValue(['time', 'value']);
  ...
  describe('formatDate', () => {
    it('should format valid dates to ISO string', () => {
      expect(DataExportService.formatDate('2023-01-01')).toBe(
        '2023-01-01T00:00:00.000Z',
      );
      expect(DataExportService.formatDate(new Date('2023-01-01'))).toBe(
        '2023-01-01T00:00:00.000Z',
      );
    });
  ...
  describe('formatFloat', () => {
    ...
    it('should format float with custom precision', () => {
      ...
      expect(DataExportService.formatFloat(123.123456789, 8)).toBe(
        123.12345679,
      );
    });
  });
